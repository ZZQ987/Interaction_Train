...
CONFIG_VARS=(
  ...
  EVAL_BEFORE_TRAIN EVAL_EVERY_STEPS EVAL_SAVE_PREDICTIONS_LIMIT
...
echo "[minicpmo_generated_rjob] EVAL_EVERY_STEPS=${EVAL_EVERY_STEPS} MAX_EVAL_BATCHES=${MAX_EVAL_BATCHES}"
...
if [[ ! -s "${TRAIN_DATA}/manifest.jsonl" ]]; then
...
echo "  eval_batch_size=${EVAL_BATCH_SIZE} eval_before_train=${EVAL_BEFORE_TRAIN} max_train_batches=${MAX_TRAIN_BATCHES}"
...
RJOB_ENV_VARS=(
  ...
  NO_SAVE_CHECKPOINTS LOG_EVERY MAX_EVAL_BATCHES KEEP_ALIVE
)
rjob_env_args=()
