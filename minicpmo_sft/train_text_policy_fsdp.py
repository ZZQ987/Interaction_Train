...
def _save_checkpoint(
    *,
    ...
    if _is_rank0():
        ...
        with (ckpt_dir / "trainer_state.json").open("w", encoding="utf-8") as fh:
            json.dump(state, fh, ensure_ascii=False, indent=2, sort_keys=True)
        logger.info(
            "checkpoint_saved epoch=%d step=%s path=%s",
            epoch,
...
def main() -> None:
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_model or args.model, trust_remote_code=True, local_files_only=True)
    processor = AutoProcessor.from_pretrained(args.processor_model or args.model, trust_remote_code=True, local_files_only=True)
    ...
    train_dataset = dataset_cls(args.train_data, **train_dataset_kwargs)
    eval_dataset = dataset_cls(args.eval_data, **eval_dataset_kwargs)
    if args.generated_trajectory_mode and _is_rank0() and hasattr(train_dataset, "filter_summary"):
    ...
    if args.train_balance_speak_listen_keep_delegate:
    ...
    model = AutoModel.from_pretrained(args.model, **model_load_kwargs)
    logger.info(
    ...
    model.train()
    logger.info(
        "model_loaded total_params=%d trainable_params=%d train_rows=%d eval_rows=%d num_frames=%d",
        total,
        trainable,
        len(train_dataset),
        len(eval_dataset),
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.eval_before_train and not did_initial_eval:
        ...
        for batch_idx, batch in enumerate(train_loader, start=1):
            ...
            should_step = batch_idx % max(1, args.grad_accum_steps) == 0 or batch_idx == len(train_loader)
            grad_norm = math.nan
            if should_step:
            ...
            if _is_rank0() and should_step and global_step > 0 and global_step % max(1, args.log_every) == 0:
                ...
                logger.info(
                    "epoch=%d batch=%d/%d global_step=%d local_loss_avg=%.6f last_loss=%.6f grad_norm=%.6f seq_len=%d peak_alloc_gb=%.3f",
                    epoch,
                    batch_idx,
                    len(train_loader),
                    global_step,
            ...
            if should_step and args.eval_every_steps > 0 and global_step > 0 and global_step % args.eval_every_steps == 0:
                eval_metrics = _evaluate(
                    model,
                    eval_loader,
                    device,
            ...
            if should_step and args.save_every_steps > 0 and global_step > 0 and global_step % args.save_every_steps == 0:
                if args.no_save_checkpoints:
                    ...
                    _save_checkpoint(
                        ...
                        checkpoint_name=f"step_{global_step:08d}",
                    )
                    if _is_rank0():
                        metrics.log(event="checkpoint_saved", **checkpoint_state, path=str(output_dir / f"step_{global_step:08d}"))
        ...
        if args.no_save_checkpoints:
            dist.barrier()
            logger.info("checkpoint_skipped epoch=%d no_save_checkpoints=true", epoch)
        else:
            _save_checkpoint(
                model=model,
                processor=processor,
